...
sequence=read_frames(ROOT/'IYTYT.mp4',min(indices),max(indices)-min(indices)+1)
source={i:sequence[i-min(indices)] for i in indices}
BASE_OUT=OUT;OUT=OUT/'v2';OUT.mkdir(exist_ok=True)
def saveim(name,im):
 from PIL import Image
 Image.fromarray(im).save(OUT/name)
ref=source[CUT]
MASKS={CUT:np.load(BASE_OUT/'mask-2888.npy')}
...
def getmask(i):
 ...
 cached=BASE_OUT/f'mask-{i}.png'
 if cached.exists():
  MASKS[i]=(cv2.imread(str(cached),cv2.IMREAD_GRAYSCALE)>127).astype(np.uint8);return MASKS[i]
...
for target in [2888,2889,2896]:
 ...
 for donor in [2887,2885,2883]:
  md=getmask(donor)
  D,dq=register(b,source[donor],m,md,'background')
  if target==2888 and donor==2887:
   D=np.linalg.inv(BG);dq['exact_inverse_cut_registration']=True
  if dq['residual_p50']>4:
   donor_report.append({'frame':donor,'pixels_used':0,'rejected':'median registration residual exceeds4px','registration':dq});continue
  M=BG@D
  can=warp((1-cv2.dilate(md,np.ones((3,3),np.uint8))).astype(np.float32),M,cv2.INTER_LINEAR)>.999
