...
D=Path(__file__).resolve().parent;OUT=D/'strong-response';OUT.mkdir(exist_ok=True);cv2.setNumThreads(2)
...
def sheet(cut,name,rect,a,b,models):
 x0,y0,x1,y1=rect;w=x1-x0;h=y1-y0;scale=2 if w<250 else 1;w*=scale;h*=scale;step=max(w,230);canvas=Image.new('RGB',(step*2+32,(h+30)*3+24),'#161616');dr=ImageDraw.Draw(canvas)
 for j,(label,model) in enumerate([('Baseline',None),('v1 limit18',models['v1']),('Strong limit63.75',models['strong'])]):
...
for cut,regions in rs.items():
 paths={'v1':ROOT/'research/local-color-fit'/f'{cut}-model.json','strong':ROOT/'research/local-color-strong'/f'{cut}-model.json'}
 if not paths['strong'].exists():continue
 ...
 for region in regions:
  ...
  if len(l):
   rr['bias_change_strong_vs_v1']=rr['strong']['mean_absolute_signed_bias']-rr['v1']['mean_absolute_signed_bias'];rr['bias_change_strong_vs_baseline']=rr['strong']['mean_absolute_signed_bias']-rr['baseline']['mean_absolute_signed_bias']
   print(cut,region['id'],'n',len(l),'base',np.round(rr['baseline']['signed_rgb_mean'],2),'v1',np.round(rr['v1']['signed_rgb_mean'],2),'strong',np.round(rr['strong']['signed_rgb_mean'],2),'dbias',round(rr['bias_change_strong_vs_v1'],3),flush=True)
  rrs.append(rr)
 for name,rect in context.get(cut,[]):sheet(cut,name,rect,a,b,models)
 rows.append({'frame':cut,'models':{n:{'path':str(p),'sha256':hashlib.sha256(p.read_bytes()).hexdigest(),'limit':models[n]['left']['limit']} for n,p in paths.items()},'regions':rrs})
