...
D=Path(__file__).resolve().parent;OUT=D/'directional-fine-response';OUT.mkdir(exist_ok=True);cv2.setNumThreads(2)
...
rs[3240].append({'id':'pants_extra','material':'Woman blue trousers, added to distinguish from jacket','left_rect_xyxy':[700,425,745,505]})
rs[3240].append({'id':'outfit_cyan_extra','material':'Whole outfit cyan color class, added for coverage','left_rect_xyxy':[663,318,770,534]})
def ed(im):
...
def sheet(cut,name,rect,a,b,models):
 x0,y0,x1,y1=rect;w=x1-x0;h=y1-y0;scale=2 if w<250 else 1;w*=scale;h*=scale;step=max(w,230);canvas=Image.new('RGB',(step*2+32,(h+30)*5+24),'#161616');dr=ImageDraw.Draw(canvas)
 for j,(label,model) in enumerate([('Baseline',None),('v1 limit18',models['v1']),('Strong limit63.75',models['strong']),('Directional gamut',models['directional']),('Directional fine',models['fine'])]):
...
for cut,regions in rs.items():
 paths={'v1':ROOT/'research/local-color-fit'/f'{cut}-model.json','strong':ROOT/'research/local-color-strong'/f'{cut}-model.json','directional':ROOT/'research/local-color-directional'/f'{cut}-model.json','fine':ROOT/'research/local-color-directional-fine'/f'{cut}-model.json'}
 if not paths['fine'].exists():continue
 ...
 for region in regions:
  ...
  if region['id']=='outfit_cyan_extra':
   mid=(l+r)/2;keep&=(mid[:,0]<75)&(mid[:,1]>100)&(mid[:,1]<170)&(mid[:,2]>170)&(mid[:,2]<230)
  ...
  if len(l):
   ...
   print(cut,region['id'],'n',len(l),'base',np.round(rr['baseline']['signed_rgb_mean'],2),'v1',np.round(rr['v1']['signed_rgb_mean'],2),'strong',np.round(rr['strong']['signed_rgb_mean'],2),'directional',np.round(rr['directional']['signed_rgb_mean'],2),'fine',np.round(rr['fine']['signed_rgb_mean'],2),'dbias_vs_dir',round(rr['fine']['mean_absolute_signed_bias']-rr['directional']['mean_absolute_signed_bias'],3),flush=True)
  rrs.append(rr)
