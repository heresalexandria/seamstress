...
const stageNames: Record<JobStage, string> = { import: 'Opening your video', detect: 'Finding the joins', analyze: 'Measuring continuity', refine: 'Refining this seam', preview: 'Making review previews', process: 'Processing your video', export: 'Exporting your film' };
...
export default function App() {
  ...
  useEffect(() => api.onJobEvent((event: JobEvent) => {
    ...
    else setNotice(event.type === 'cancelled' ? 'Processing stopped. Your source video is unchanged.' : event.stage === 'refine' ? 'Selected seam refined. The viewing crop and other corrections are preserved. Preview this seam to review the change.' : event.stage === 'export' ? 'Export complete. Your film is ready to review.' : event.stage === 'process' && event.project?.artifacts.export ? 'Workflow complete. Your export is ready to review.' : 'Stage complete. Your project has been updated.');
  }), [applyProject]);
  ...
  async function runStage(stage: Stage, exportPath?: string, selectedOnly = false) {
    ...
    try {
      const result = await api.run({ projectPath: project.projectPath, stage, options: { crf, previewSeconds, ...(seamOnly ? { frame: selected!.frame } : {}), ...(stage === 'refine' && supportFrames !== undefined ? { supportFrames } : {}), ...(exportPath ? { exportPath } : {}) } });
